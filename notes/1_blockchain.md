...
- [CAP定理](#cap定理)
- [拜占庭将军问题](#拜占庭将军问题)
- [比特币：第一个区块链](#比特币第一个区块链)
- [共识机制](#共识机制)
  - [工作量证明（PoW）](#工作量证明pow)
  - [权益证明（PoS）](#权益证明proof-of-stake)
- [区块链三难问题](#区块链三难问题)
- [密码学原语](#密码学原语cryptographic-primitives)
  - [哈希函数](#1-哈希函数-hash-function)
  - [非对称加密与数字签名](#2-非对称加密与数字签名-asymmetric-encryption--digital-signature)
  - [默克尔树](#3-默克尔树-merkle-tree)
  - [同态加密与零知识证明](#4-同态加密与零知识证明-advanced-primitives)
- [创建一个无需信任的系统](#创建一个无需信任的系统)
...
- **传统银行系统**：选择一致性 + 分区容错性（CP 系统）。它们宁愿关闭系统也不愿显示错误的账户余额。
- **社交媒体平台**：选择可用性 + 分区容错性（AP 系统）。它们宁愿让你发布内容（即使朋友无法立即看到）也不愿完全阻止你发布。
...
**核心问题：** 当你无法区分忠诚的将军和叛徒，也无法信任通信渠道时，如何就"进攻"或"撤退"达成共识？
...
1. **收集交易**：矿工将待处理的交易收集到一个"区块"中
2. **寻找随机数**：矿工必须找到一个随机数（称为"nonce"），当与区块数据结合并进行哈希运算时，产生以多个零开头的结果
3. **广播区块**：第一个找到该数字的矿工将其解决方案广播到网络
4. **验证接受**：其他参与者可以立即验证解决方案的正确性并接受新区块

**关键特性：**

- **计算不对称性**：找到随机数可能需要数万亿次随机猜测，但验证解决方案只需几毫秒
- **链式结构**：每个区块还引用了前一个区块的哈希值，从而形成了一条链
- **安全性**：要篡改历史，攻击者需要重新完成所有后续的计算工作，而诚实的矿工会继续扩展真实的链
- **经济假设**：攻击的电力成本高于攻击者可能获得的收益
...
1. **质押代币**：参与者将加密货币代币锁定作为抵押
2. **选择验证者**：协议根据其权益随机选择验证者提议区块
3. **提议和投票**：被选中的验证者提议区块，其他验证者投票接受或拒绝
4. **奖惩机制**：诚实行为会获得奖励；不诚实行为会导致"削减"（Slashing），即部分质押的代币被没收或罚没

**关键优势：**

- **利益相关**：验证者有"利益相关"，攻击网络会破坏其质押代币的价值
- **经济终局性**：与工作量证明不同，权益证明可以提供经济终局性。一旦区块被绝大多数验证者最终确定，攻击者要想逆转它，就需要证明性地销毁大量资本，使得逆转成本高得无法承受
- **能源效率**：不需要消耗大量电力进行计算
...
- **比特币**：选择了安全性和去中心化，而不是可扩展性
- **Visa 等传统支付系统**：选择了可扩展性和安全性，而不是去中心化
- **当前挑战**：找到同时实现这三者的方法
...
证明者能在不向验证者提供任何有用信息的情况下，使验证者相信某个论断是正确的。

- **例子**：证明你有超过 100 个比特币，但不用展示你的余额或转账记录

**同态加密 (Homomorphic Encryption)**

允许直接对加密数据进行运算，得到的结果解密后与对明文运算的结果一致。
...
- **哈希函数**：确保任何对历史数据的篡改都会立即显现
- **数字签名**：在无需任何可信中介验证身份的情况下证明授权
- **默克尔树**：使得无需下载大量数据即可验证复杂声明成为可能
...
理解这些基础要素至关重要，因为它们定义了区块链的能力和局限性。它们解释了：

- **为什么区块链交易是不可逆的**：设计上使得逆转已完成交易的经济成本极高
- **为什么区块链系统可以在没有中心化权威的情况下运行**：每个人都可以独立验证所有内容
- **为什么即使完全向公众开放参与，系统仍然保持安全**：数学和密码学保证了系统的安全性
